...
8. **Tetos de custo em US$** (controle primário): global **mensal** `AGENT_MONTHLY_COST_BUDGET_USD`
   (default US$ 20/mês — o orçamento de fato, "não passar disso contando tudo") e por vendedor
   **diário** `AGENT_DAILY_COST_PER_SELLER_USD` (default US$ 1/dia, para um vendedor não abusar do
   chat num único dia). Atingido um teto, o agente **degrada** (última resposta válida + aviso) em
   vez de estourar o orçamento, e registra alerta (grupo IA, doc 09) — com aviso ao cruzar 80%.
   Backstop absoluto em tokens/dia: `AGENT_DAILY_TOKEN_BUDGET`.
